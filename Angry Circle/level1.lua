...
local function endfunction()
	...
	if died == false then
	timer.performWithDelay(1000, function() composer.gotoScene("level2")end)
	else
	objectX = 0 
...
local function jump()
	...
	circle:applyLinearImpulse(0, -0.29, circle.x, circle.y)
	end
end

local function jumpReset(event)
	if event.phase == "began" then
	jumped = false
 	circle:setLinearVelocity(.1, circle.VelocityY)
	end
end
...
function scene:show( event )
	...
	elseif phase == "did" then
		...
		Runtime:addEventListener("collision", jumpReset)
...
function scene:destroy( event )
	...
	Runtime:removeEventListener("collision", jumpReset)
